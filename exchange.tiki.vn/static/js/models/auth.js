...
import { initClient as initSsoClient } from '@tikivn/tiki-sso-client';
import { safelyParseJson } from '@utils';
import * as RudderStack from '@tikivn/stackity-web-sdk';

import authService from '../services/auth';
import { getBalances, getUserInfo } from '../services/me';
...
export default {
  ...
  effects: {
    ...
    *login({ payload: { isRegister = false } = {} }, { call, put }) {
    ...
    sessionWatcher: [
      function* ({ take, put }) {
        while (true) {
          const { payload } = yield take('saveSession');
          if (payload) {
            yield put({
              type: 'fetchUserInfo',
            });
    ...
    *fetchUserInfo(_, { call, put, all }) {
      try {
        const [userInfo, { balances } = {}] = yield all([
          call(getUserInfo),
          call(getBalances),
        ]);
